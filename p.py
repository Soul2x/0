...
# Function to join chat with a countdown
def join_chat_repeatedly():
    global stop_leaving
    try:
        while True:
            if ncd_client and thread_id and not stop_leaving:
                ...
                for i in range(3, 0, -1):
                    ...
                    time.sleep(1)
                console.print("[bold green]Joining the chat...", end='\r')
                time.sleep(1)  # Briefly show the join message before clearing it
                ncd_client.join_chat(chatId=thread_id)
                console.print("[bold green]Joined the chat.", end='\r')
            time.sleep(3)
...
# Start the join and leave chat processes
threading.Thread(target=join_chat_repeatedly, daemon=True).start()
threading.Thread(target=leave_chat_repeatedly, daemon=True).start()
